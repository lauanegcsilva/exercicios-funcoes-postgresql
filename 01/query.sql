...
select sum(estoque) as estoqueBlueBlack from farmacia where categoria in ('black', 'blue') ;
...
select categoria, count(estoque) as quantidadeMedicamentos from farmacia  where categoria is null group by categoria;
...
select concat(medicamento, ' (', categoria, ') ') as medicamentoCategoria from farmacia where categoria is not null;

-- Faça uma query que retorna uma única coluna com a junção do identificador e o nome do medicamento separado por um hífen e a categoria entre parenteses de todos os registros da tabela farmacia. Quando a categoria for nula, substituir por (sem categoria). Ex.: 1 - Endocet (green) ou 1 - Endocet (sem categoria).
...
select  nome, idade, cast(usuarios.date as date) from usuarios where  usuarios.date::date < '2021-01-01' and usuarios.date::date > '2019-12-31';
...
select  nome, idade, email, age(cast(usuarios.date as timestamp)) from usuarios where  idade<= 18  ;
...
select  nome, idade, email, age(cast(usuarios.date as timestamp)) as tempoCadastro from usuarios where idade>= 60;
